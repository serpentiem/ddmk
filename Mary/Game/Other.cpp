...
void Game_Other_Update()
{
	...
	// Orb Reach
	{
		*orbReach = Config.Game.Other.orbReach;
		WriteAddress((appBaseAddr + 0x1B655F), (byte8 *)orbReach, 8);
	}
	// Magic Points Depletion Rate
	{
		*MagicPointsDepletionRate_devil = Config.Game.Other.MagicPointsDepletionRate.devil;
		Write<byte8>((appBaseAddr + 0x1F8B4C), 0x0D);
		WriteAddress((appBaseAddr + 0x1F8B49), (byte8 *)MagicPointsDepletionRate_devil, 8);
		*MagicPointsDepletionRate_quicksilver = Config.Game.Other.MagicPointsDepletionRate.quicksilver;
		Write<byte8>((appBaseAddr + 0x1F8A43), 0x0D);
		WriteAddress((appBaseAddr + 0x1F8A40), (byte8 *)MagicPointsDepletionRate_quicksilver, 8);
		*MagicPointsDepletionRate_doppelganger = Config.Game.Other.MagicPointsDepletionRate.doppelganger;
		Write<byte8>((appBaseAddr + 0x1F89D4), 0x0D);
		WriteAddress((appBaseAddr + 0x1F89D1), (byte8 *)MagicPointsDepletionRate_doppelganger, 8);
	}
}
