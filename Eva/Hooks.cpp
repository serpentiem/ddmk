...
DWORD AcquireMouseThread(LPVOID parameter)
{
	do
	{
		LoopStart:
...
DWORD DirectInput8::CreateMouse(LPVOID parameter)
{
	...
	CreateThread(0, 4096, AcquireMouseThread, 0, 0, 0);
...
void Hooks::Init()
{
	...
	Write<DWORD>((appBaseAddr + 0x18754), DISCL_NONEXCLUSIVE | DISCL_FOREGROUND); // Keyboard Cooperative Level
	{
		BYTE payload[] =
		{
			...
			0x59,                                                       //pop rcx
			0x58,                                                       //pop rax
			0xE9, 0x00, 0x00, 0x00, 0x00,                               //jmp dmc1.exe+156D3
		};
		...
		*(void **)(func.addr + 0x2B) = DirectInput8::Hook::GetDeviceStateKeyboard;
		WriteJump((func.addr + 0x3E), (appBaseAddr + 0x156D3));
	...
	}
	{
		BYTE payload[] =
		{
			...
			0x59,                                                       //pop rcx
			0x58,                                                       //pop rax
			0xE9, 0x00, 0x00, 0x00, 0x00,                               //jmp dmc1.exe+189E5
		};
		FUNC func = CreateFunction(0, 0, false, false, sizeof(payload));
		memcpy(func.addr, payload, sizeof(payload));
		*(void **)(func.addr + 0x2B) = DirectInput8::Hook::GetDeviceStateKeyboard;
		WriteJump((func.addr + 0x3E), (appBaseAddr + 0x189E5));
	...
	}
	{
		BYTE payload[] =
		{
			...
			0x59,                                                       //pop rcx
			0x58,                                                       //pop rax
			0xE9, 0x00, 0x00, 0x00, 0x00,                               //jmp dmc1.exe+154CF
		};
		FUNC func = CreateFunction(0, 0, false, false, sizeof(payload));
		memcpy(func.addr, payload, sizeof(payload));
		WriteAddress((func.addr + 0xF), (appBaseAddr + 0x4073F8), 6);
		*(void **)(func.addr + 0x23) = XInput::Hook::GetState;
		WriteJump((func.addr + 0x34), (appBaseAddr + 0x154CF));
