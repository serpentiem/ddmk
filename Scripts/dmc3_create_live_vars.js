...
c += "<CheatEntry>\n";
c += "<Description>\"System_Actor_actorBaseAddr\"</Description>\n";
c += "<Options moHideChildren=\"1\"/>\n";
c += "<GroupHeader>1</GroupHeader>\n";
c += "<CheatEntries>\n";
...
for (var actor = 0; actor < 4; actor++)
{
	...
	for (var itemIndex = 0; itemIndex < items.length; itemIndex++)
	{
		...
		if (type == "vec4")
		{
			AddCheatEntry("x", false, "Float", 0, (off + 0  ), actor, true);
			AddCheatEntry("y", false, "Float", 0, (off + 4  ), actor, true);
			AddCheatEntry("z", false, "Float", 0, (off + 8  ), actor, true);
			AddCheatEntry("a", false, "Float", 0, (off + 0xC), actor, true);
			continue;
		}
		else if (type == "INPUT_BUFFER")
		{
			continue;
		}
		else if (type == "MOTION_DATA")
		...
		for (var index = 0; index < count; index++)
		{
			...
			AddCheatEntry(description, GetShowAsHex(type), GetVariableType(type), 0, off, actor, true);
		}
	}
	c += "</CheatEntries>\n";
	c += "</CheatEntry>\n";
